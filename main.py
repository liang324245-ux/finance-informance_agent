...
请保留涉及以下内容的新闻：技术突破/产品发布、资金投入/扩产计划、融资动作、上市/IPO动作、重大合作或订单、业绩数据、产能变化。

请丢弃：广告、软文、PR稿、无实质内容的会议预告、与公司核心业务无关的内容。

【去重规则】同一公司的同一类事件只保留信息量最大的1条：
- 同一公司的多条融资/扩产新闻 → 只留金额/规模最大或最新的1条
- 同一公司的多条产品发布新闻 → 只留信息最全的1条
- 同一公司被多家媒体重复报道同一事件 → 只留1条
- 单个层内同一公司最多保留2条（需事件类型明显不同才可并存）
...
【信息来源说明】今天的新闻来自六个各有��重的源，请结合来源性质做判断：
...
- 国���统计局-数据发布：中国官方统计机构直接发布的月度/季度经济数据，包括PMI（每月最后一天）、CPI/PPI（每月中旬）、GDP（每季度初）、工业利润、社融等核心指标。此类数据是宏观判断的原始依据，无论标题是否有分析深度，只要是新发布的指标数据，一律保留，不得以"内容简短"为由丢弃。
【保留标准】保留满足以下任一条件的新闻：
...
8. A股市场整体资金流向与风格切换（板块轮动、主力资金净流入/流出统计、机构调研动向、南向/北向资金流向、市场情绪指标）——判断标准：反映的是板块或整���市场层面的资金行为，而非单一股票的涨跌消息。例如"券商保险板块拉升，科技股跳水""7股遭主力资金大幅出逃""南向资金6月净流入271亿港元"应保留。
【去重规则（严格执行）】
...
- 同一场合/同一次发言被拆分成多篇独立报道时（如同一场论坛发言被按话题拆成多篇稿件）：
  第一步：从中挑出信息量最大、市场影响最直接的1-2条作为精选新闻正式列出，标题和链接必须是���文，不可改写标题。
...
- 其他国家：仅保留具有全球性宏观影响的内容（如欧洲央行/日本央行利率决议、影响全球供应链的事件）
- 丢弃：他国本国价格管控、补贴、社会政策
...
def fetch_one_source(name, url):
    ...
    try:
        ...
        LOW_FREQ_SOURCES = {"国家统计局-数据发布"}
        run_mode = os.getenv("RUN_MODE", "morning")  # 默认早报模式(48小时)，晚报模式��evening(13小时)
...
def send_to_feishu(text, max_retries=2):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"❌ 发送出��：{type(error).__name__} - {error}")
            return False
    print("❌ 飞书发送多次重试后仍失败")
    return False
